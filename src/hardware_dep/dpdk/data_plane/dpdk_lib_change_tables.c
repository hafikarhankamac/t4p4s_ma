...
#define FORALL_PRINTOUT(txt1, txt2, b, is_const_entry, should_print) \
    ...
    if (should_print || T4P4S_FORCE_PRINT) { \
        ...
        dbg_bytes(key, entry_info.key_size, " " T4LIT(HAK-ctl>,incoming) " " txt1 " " T4LIT(%s,table) txt2 ": " T4LIT(%s,action) "%s <- ", tcfg.short_name, get_entry_action_name(value), params_str); \
    }
#else
#define FORALL_PRINTOUT(txt1, txt2, b, is_const_entry, should_print)
#endif
