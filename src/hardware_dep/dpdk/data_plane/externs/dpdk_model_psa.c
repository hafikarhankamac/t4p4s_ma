...
int extract_egress_port(packet_descriptor_t* pd) {
    return GET_INT64_AUTO_PACKET(pd, HDR(all_metadatas), EGRESS_META_FLD);
}

int extract_ingress_port(packet_descriptor_t* pd) {
    return GET_INT64_AUTO_PACKET(pd, HDR(all_metadatas), INGRESS_META_FLD);
}

void set_handle_packet_metadata(packet_descriptor_t* pd, uint32_t portid)
{
    int res32; // needed for the macro
    MODIFY_INT64_INT64_BITS_PACKET(pd, HDR(all_metadatas), INGRESS_META_FLD, portid);
}
