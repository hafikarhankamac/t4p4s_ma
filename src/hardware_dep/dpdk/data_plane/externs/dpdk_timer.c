...
void timer_callback_burst(struct rte_timer *tim, void *arg)
{
    timer_event_t *timer_event = (timer_event_t*) arg;
    fill_event_queue(lcore_conf[timer_event->lcore].state.event_queue, TIMER, timer_event->id, MAX_EVENT_BURST - 1);
}
...
void burst_timer_lcore(uint32_t id, uint32_t lcore)
{
	if (!already[lcore]) {
	    timer_event_t *timer_event = rte_malloc("timer", sizeof(timer_event_t), 0);
	    struct rte_timer *rtetimer = rte_malloc("rte_timer", sizeof(struct rte_timer), 0);
	    ...
	    timer_event->id = id;
	    rte_timer_init(rtetimer);
	    rte_timer_reset_sync(rtetimer, 1, PERIODICAL, lcore, timer_callback_burst, (void*) timer_event);
	    already[lcore] = true;
	}
}
