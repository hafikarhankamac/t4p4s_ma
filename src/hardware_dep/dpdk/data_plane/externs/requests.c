...
{
   request_t *req = rte_malloc("request_to_store_t", sizeof(request_to_store_t), 0);
   req->request->payload = { .req = req, .args = args};
   ...
   rte_add_key_data(rs->snlv, snlv, req);

   max_not_executed = max_not_executed > sn ? max_not_executed : sn;
}

inline uint64_t get_sn_lv_key(uint32_t sn, uint32_t lv) {
    uint64_t snlv = sn << 32;
    snlv |= lv;
    return snlv;
}

inline void hash_request(request_t *req, uint8_t *hash) {
   uint8_t key[5];
   key[0] = req->req;
   key[1] = req->arg;
   *hash = hash(key, 5);
...
//TODO delivered=true
void extern_request_store_commit(uint32_t declarg, digest_t digest, uint32_t last_exec, request_store_t *rs, SHORT_STDPARAMS)
{
    ...
    for (uint32_t i = rs->min_not_executed; i <= max; i++) {
        ...
        if (req->request.processed) {
            min_not_executed = r->sn;
        } else {
            raise_event(PROCESS_REQUEST, r->request.dig);
...
{
    struct request = {.req = req, .args = args};
    hash(&request, *dig);
}
