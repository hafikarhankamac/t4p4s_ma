...
int process_ternary(const char* line) {
    ...
    int matches = sscanf(line, "%*s %s %hhd.%hhd.%hhd.%hhd %hhx%hhx%hhx%hhx %d", table_name, &ip[0], &ip[1], &ip[2], &ip[3], &mask[0], &mask[1], &mask[2], &mask[3], &priority);

    printf("IP: %hhd.%hhd.%hhd.%hhd Mask: %hhx%hhx%hhx%hhx Priority: %d\n", ip[0], ip[1], ip[2], ip[3], mask[0], mask[1], mask[2], mask[3], priority);
