...
def gen_format_expr(e, format_as_value=True, expand_parameters=False):
    ...
    # TODO some of these are formatted as statements, we shall fix this
        ...
        if method:
		    ...
		    while e.typeArguments.vec[index].size > required_size and required_size<64:
		        required_size += 8
		...
		with types({
			...
			#pre[ extern void hash_b${base_size_bit}_m${max_size_bit}(uint8_t*, int, int, ${type_env['T']}${base_array}, ${type_env['D']}, ${type_env['M']}${max_array}, SHORT_STDPARAMS);
